import { PreferencesAction, setPreferenceActionReducer } from '@/preferences/actions';
import { Preferences } from '@/preferences/prefs-data';
import { Reducer } from 'react';

type PrefsReducer = Reducer<Preferences, PreferencesAction>;

const ACTION_TYPE_TO_REDUCER_MAP: Record<PreferencesAction['type'], PrefsReducer> = {
  'set-preference': setPreferenceActionReducer,
};

function getActionReducerFromActionType(actionType: PreferencesAction['type']): PrefsReducer {
...
export function preferencesReducer(prevState: Preferences, action: PreferencesAction) {
